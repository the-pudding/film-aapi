  ...
  function parseRows(text, f) {
    ...
    function token() {
      ...
      if (text.charCodeAt(j) === QUOTE2) {
        ...
        if ((i = I) >= N) eof = true;
        else if ((c4 = text.charCodeAt(I++)) === NEWLINE2) eol = true;
        else if (c4 === RETURN2) {
          eol = true;
      ...
      while (I < N) {
        if ((c4 = text.charCodeAt(i = I++)) === NEWLINE2) eol = true;
        else if (c4 === RETURN2) {
          eol = true;
          if (text.charCodeAt(I) === NEWLINE2) ++I;
        } else if (c4 !== DELIMITER) continue;
...
function circle_default2(radius) {
  ...
  function intersect2(a4, b, two) {
    ...
    var delta2 = lambda12 - lambda04, polar = abs3(delta2 - pi4) < epsilon5, meridian = polar || delta2 < epsilon5;
    if (!polar && phi12 < phi02) z = phi02, phi02 = phi12, phi12 = z;
    if (meridian ? polar ? phi02 + phi12 > 0 ^ q[1] < (abs3(q[0] - lambda04) < epsilon5 ? phi02 : phi12) : phi02 <= q[1] && q[1] <= phi12 : delta2 > pi4 ^ (lambda04 <= q[0] && q[0] <= lambda12)) {
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b02, c0, x12, y12, lambda12, a1, b12, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a4 = a0 + a1, b = b02 + b12, c4 = c0 + c1, m3 = sqrt2(a4 * a4 + b * b + c4 * c4), phi2 = asin(c4 /= m3), lambda22 = abs3(abs3(c4) - 1) < epsilon5 || abs3(lambda04 - lambda12) < epsilon5 ? (lambda04 + lambda12) / 2 : atan2(b, a4), p = project(lambda22, phi2), x22 = p[0], y22 = p[1], dx2 = x22 - x06, dy2 = y22 - y06, dz = dy * dx2 - dx * dy2;
...
function conicConformalRaw(y06, y12) {
  ...
  function project(x4, y4) {
    if (f > 0) {
      if (y4 < -halfPi3 + epsilon5) y4 = -halfPi3 + epsilon5;
    } else {
      if (y4 > halfPi3 - epsilon5) y4 = halfPi3 - epsilon5;
    }
...
function pie_default() {
  var value = identity_default3, sortValues = descending_default, sort2 = null, startAngle = constant_default8(0), endAngle = constant_default8(tau6), padAngle = constant_default8(0);
  function pie(data) {
